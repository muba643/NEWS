...
// ---- NEWS FEED JSON (for the mobile app) ----
function loadNewsFeed() {
  if (!fs.existsSync(NEWS_FEED_FILE)) return [];
  try {
    return JSON.parse(fs.readFileSync(NEWS_FEED_FILE, "utf-8"));
  } catch {
    return [];
  }
}

function saveNewsFeed(feedList) {
  // Keep newest items first, capped at MAX_FEED_ITEMS
  const trimmed = feedList.slice(0, MAX_FEED_ITEMS);
  fs.writeFileSync(NEWS_FEED_FILE, JSON.stringify(trimmed, null, 2));
}
...
async function run() {
  ...
  for (const feed of FEEDS) {
    try {
      ...
      for (const item of latestItems) {
        ...
        // Add this item to the public news feed JSON for the mobile app
        newFeedItems.push({
          id: Buffer.from(item.link).toString("base64").slice(0, 24),
          title: item.title,
          english: ai?.english || item.title,
          amharic: ai?.amharic || null,
          oromo: ai?.oromo || null,
          image: image || null,
          source: feed.name,
  ...
  if (newLinks.length > 0) {
    ...
    saveNewsFeed([...newFeedItems, ...newsFeed]);
